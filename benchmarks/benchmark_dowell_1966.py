...
def print_simulation_setup(
    ...
    if basis_type.name == "BARDELL":
        logger.info(
            f" {'Boundary Conditions':<25} : {boundary_conditions.name}"
        )
    else:
        logger.info(
    ...
    logger.info(f" {'Approximation Functions':<25} : {n_func}")
...
def run_simulations(
    ...
    output_dir = CURRENT_DIR / "figures" / "compare_Dowell" / subcase
    ...
    E1 = MAT_REF.E if hasattr(MAT_REF, "E") else MAT_REF.nu21
    nu12 = MAT_REF.nu if hasattr(MAT_REF, "nu") else MAT_REF.nu12
    nu21 = MAT_REF.nu if hasattr(MAT_REF, "nu") else MAT_REF.nu21
    D011 = E1 * h**3 / 12 / (1 - nu12 * nu21)
    w0 = (D011 / (MAT_REF.rho * h * panel.a**4)) ** 0.5
